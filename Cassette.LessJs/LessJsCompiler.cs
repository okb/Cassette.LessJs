...
namespace Cassette.Stylesheets
{
    public class LessJsCompiler : ILessJsCompiler
    {
        ...
        public CompileResult Compile(string source, CompileContext context)
        {
            ...
            try
            {
                ...
                applicationRootDirectory =
                    (string)
                        rootDirectory.GetType().GetField("fullSystemPath",
                            BindingFlags.NonPublic | BindingFlags.GetField | BindingFlags.Instance).GetValue(
                                rootDirectory);
                applicationRootDirectory = string.Format("{0}{1}",
                    applicationRootDirectory[0].ToString().ToLowerInvariant(), applicationRootDirectory.Substring(1));
            }
            catch
            {
            }
        ...
        private CompileResult Compile(IFile source)
        {
            ...
            string absolutePath =
                (string)
                    source.GetType().GetField("systemAbsoluteFilename",
                        BindingFlags.NonPublic | BindingFlags.GetField | BindingFlags.Instance).GetValue(source);
            ProcessStartInfo start = new ProcessStartInfo(@"cscript");
            start.WindowStyle = ProcessWindowStyle.Hidden;
            start.CreateNoWindow = true;
            start.Arguments = "//nologo \"" + GetExecutablePath() + "\" \"" + absolutePath + "\" \"" + output + "\"" +
                              " -filenames";
            ...
            while (compileResult == null && compileException == null &&
                   (elapsedTime = elapsedTime.Add(new TimeSpan(0, 0, 0, 0, SLEEP_AMOUNT))).TotalSeconds <= 30)
        ...
        private IFile EnsureExists(string source, IFile sourceFile, IDirectory rootDirectory)
        {
            ...
            if (!sourceFile.Exists)
            {
                using (var reader = sourceFile.OpenRead())
                {
                    if (reader == null)
                    {
                        Copy(sourceFile, destinationDirectoryInfo.Item1,
        ...
        private void Marshal(IDirectory source, IDirectory destination, IFile excludeFileFromCopy)
        {
            ...
            foreach (
                var sourceItem in
                    source.GetFiles("*.*", SearchOption.AllDirectories).Where(f => f != excludeFileFromCopy))
            {
                ...
                if (process.ExitCode == 0)
                {
                    using (var reader = process.StandardOutput)
                    {
                        paths = reader.ReadToEnd().Split('\n')
                            ...
                            .Select(
                                s =>
                                    s.Replace("\r", "").Replace("\\", "/").Replace(applicationRootDirectory ?? "~", "~"));
                    }
                }
                else
                {
        ...
        #region Nested type: CompilerError
